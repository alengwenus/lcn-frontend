...
import { LCN, fetchDevices, fetchEntities, LcnConfig, LcnEntityConfig, addDevice, addEntity } from "types/lcn"
...
export async function exportConfig(hass: HomeAssistant, lcn: LCN) {
    ...
    for await (const device of config.devices) {
      const device_entities: LcnEntityConfig[] = await fetchEntities(hass!, lcn.config_entry, device.address)
...
export async function importConfig(hass: HomeAssistant, lcn: LCN) {
    lcn.log.debug("Importing config");
    const file: File = await openFileDialog();
    const config: LcnConfig = await readConfigFile(file);
    lcn.log.debug(config);

    lcn.log.debug("Adding devices");
    for await (const device of config.devices) {
        const result = await addDevice(hass, lcn.config_entry, device);
        lcn.log.debug(result);
    }

    lcn.log.debug("Adding entities");
    for await (const entity of config.entities) {
        lcn.log.debug(entity);
        const result = await addEntity(hass, lcn.config_entry, entity);
        lcn.log.debug(result);
    }
}
